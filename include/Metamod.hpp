...
namespace Metamod
{
    class Metamod : public IMetamod
    {
    public:
        Metamod();

        std::uint32_t getInterfaceVersion() const override;
        Engine::Engine *getEngine() const override;
        GameLib::GameLib *getGameLib() const override;
        GameType getGameType() const override;
        const RegMsg *getMsgInfo(std::string_view name) const override;

        bool addNewMsg(std::uint8_t id, std::string_view name, std::int16_t size);

    private:
        bool _findMessage(std::uint8_t id);
        void _loadPlugins(std::string_view gameDir);
