...
namespace Metamod::Engine::Callbacks::GameDLL
{
    ...
    {
        gEngine->queryClientCvarValue(gEngine->getEdict(const_cast<edict_t *>(player)), cvarName, FuncCallType::Hooks);
    }

    void pfnQueryClientCvarValue2(const edict_t *player, const char *cvarName, int requestID)
    {
        gEngine->queryClientCvarValue2(gEngine->getEdict(const_cast<edict_t *>(player)),
                                      cvarName,
