...
namespace
{
    template<typename T, typename U, typename = std::enable_if_t<std::conjunction_v<
        std::is_base_of<Game::Entities::IBaseEntity, T>,
...
namespace Metamod::Game::VFunc
{
    void vCBasePlayerSpawn(IReGameHook_CBasePlayer_Spawn *hook, CBasePlayer *player)
    {
        ...
        hookchain->callChain([player, hook](Entities::IBasePlayer *) {
            hook->callNext(player);
        }, [player, hook](Entities::IBasePlayer *) {
            hook->callOriginal(player);
        }, instanceToType<Entities::IBasePlayer>(player));
    }
                                    ...
                                    std::int32_t bitsDamageType)
    {
        ...
        return hookchain->callChain(
            [player, hook](Game::Entities::IBasePlayer *, Engine::IEntVars *pevInflictor,
            ...
            instanceToType<Game::Entities::IBasePlayer>(player), gEngineLib->getEntVars(pevInflictor),
 ...
 * bitsDamageType); */
        ...
        std::int32_t bitsDamageType)
    {
        ...
        hookchain->callChain([hook, player, &vecDir] (Game::Entities::IBasePlayer *, Engine::IEntVars *pevAttacker,
                                            float flDamage, float *, Engine::ITraceResult *metatr,
                                            std::int32_t bitsDamageType) {
            hook->callNext(player, *pevAttacker, flDamage, vecDir, *metatr, bitsDamageType);
        }, [hook, player, &vecDir] (Game::Entities::IBasePlayer *, Engine::IEntVars *pevAttacker,
                                    float flDamage, float *, Engine::ITraceResult *metatr,
                                    std::int32_t bitsDamageType) {
            hook->callOriginal(player, *pevAttacker, flDamage, vecDir, *metatr, bitsDamageType);
        },
        instanceToType<Game::Entities::IBasePlayer>(player), gEngineLib->getEntVars(pevAttacker),
